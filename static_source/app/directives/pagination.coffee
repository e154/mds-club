...
angular.module('template/pagination/pagination.html', []).run [
  '$templateCache'
  ($templateCache) ->
    $templateCache.put 'template/pagination/pagination.html', '<div class="ui borderless pagination menu">\n' + '<a ng-if="boundaryLinks" class="item" ng-class="{disabled: noPrevious()}" href="" ng-click="selectPage(1)">{{getText("first")}}</a>\n' + '<a ng-if="directionLinks" class="item" ng-class="{disabled: noPrevious()}" href="" ng-click="selectPage(page - 1)">{{getText("previous")}}</a>\n' + '<a ng-repeat="page in pages track by $index" ng-class="{active: page.active}" href="" class="item" ng-click="selectPage(page.number)">{{page.text}}</a>\n' + '<a ng-if="directionLinks" class="item" ng-class="{disabled: noNext()}" href="" ng-click="selectPage(page + 1)">{{getText("next")}}</a>\n' + '<a ng-if="boundaryLinks" class="item" ng-class="{disabled: noNext()}" href="" ng-click="selectPage(totalPages)">{{getText("last")}}</a>\n' + '</div>'
    return
]
